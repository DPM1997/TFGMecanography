...
public class ClickU : MonoBehaviour
{
    bool inside;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.U) && inside == true){
          ...
          inside=false;
        } else if(Input.GetKeyDown(KeyCode.U) && inside == false){
            score = Int32.Parse(text.text);
            score = score -1;
            text.text=(""+score);
        }
