...
public class ClickZ : MonoBehaviour
{
    bool inside;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Z) && inside == true){
          ...
          inside=false;
        } else if(Input.GetKeyDown(KeyCode.Z) && inside == false){
            score = score -1;
            text.text=(""+score);
        }
