...
public class ClickN : MonoBehaviour
{
    bool inside;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.N) && inside == true){
          ...
          inside=false;
        } else if(Input.GetKeyDown(KeyCode.N) && inside == false){
            score = Int32.Parse(text.text);
            score = score -1;
            text.text=(""+score);
        }
